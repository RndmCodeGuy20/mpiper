...
def process_video_file(asset_id, local_raw_path, pg_pool: PgPool, storage: StorageX, cfg):
    ...
    with pg_pool.get_conn() as conn:
        cur = conn.cursor()
        cur.execute("UPDATE assets SET width = %s, height = %s, duration_seconds = %s, mime_type = %s, updated_at=now() WHERE asset_id=%s",
                    (width, height, int(duration) if duration else None, "video", asset_id))

    # 1) create poster
    poster_path = os.path.join(tmp_dir, f"{asset_id}-poster.jpg")
    try:
        _run(["ffmpeg","-ss","2","-i", local_raw_path, "-frames:v","1","-vf","scale=640:-1", poster_path, "-y"])
        with open(poster_path,"rb") as f:
            data = f.read()
            key = f"media/processed/{asset_id}/video/poster_640.jpg"
            url = storage.upload_bytes(key, data, content_type="image/jpeg")
            with pg_pool.get_conn() as conn:
                cur = conn.cursor()
                cur.execute("""
    ...
    try:
        _run(["ffmpeg","-i", local_raw_path,
              "-c:v","libx264","-preset","veryfast","-crf","23",
              "-vf", scale_filter, "-c:a","aac","-b:a","128k", out_720, "-y"])
        with open(out_720,"rb") as f:
            data = f.read()
            key = f"media/processed/{asset_id}/video/stream_720p.mp4"
            url = storage.upload_bytes(key, data, content_type="video/mp4")
            with pg_pool.get_conn() as conn:
                cur = conn.cursor()
                cur.execute("""
                            INSERT INTO variants.video (variant_id, asset_id, role, codec, container, resolution, bitrate_kbps, url, size_bytes, created_at)
                            VALUES (gen_random_uuid(), %s, 'stream', 'h264', 'mp4', %s, %s, %s, %s, now())
                                ON CONFLICT (asset_id, role) DO NOTHING
                            """, (asset_id, f"{1280}x{720}", 2500, url, len(data)))
    except Exception:
    ...
    try:
        _run(["ffmpeg","-ss","2","-t","4","-i", local_raw_path, "-an",
              "-vf","scale=854:-2", "-c:v","libx264","-preset","veryfast","-crf","25", preview_out, "-y"])
        with open(preview_out,"rb") as f:
            ...
            with pg_pool.get_conn() as conn:
                ...
                cur.execute("""
                            INSERT INTO variants.video (variant_id, asset_id, role, codec, container, resolution, bitrate_kbps, url, size_bytes, created_at)
                            VALUES (gen_random_uuid(), %s, 'preview', 'h264', 'mp4', %s, %s, %s, %s, now())
                                ON CONFLICT (asset_id, role) DO NOTHING
                            """, (asset_id, "854x480", 800, url, len(data)))
    except Exception:
