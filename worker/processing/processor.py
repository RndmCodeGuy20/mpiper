...
def process_asset_dispatch(asset_id, pg_pool: PgPool, storage: StorageX, cfg: WorkerConfig):
    ...
    with pg_pool.get_pg_conn() as conn:
        cur = conn.cursor()
        cur.execute("SELECT asset_id, type, status, original_url, mime_type, content_hash FROM assets WHERE asset_id = %s", (asset_id,))
        row = cur.fetchone()
        if not row:
    ...
    if content_hash is None:
        ...
        try:
            with pg_pool.get_pg_conn() as conn:
                cur = conn.cursor()
                cur.execute("UPDATE assets SET content_hash = %s WHERE asset_id = %s", (computed_hash, asset_id))
            logger.info("computed and stored content hash for asset %s: %s", asset_id, computed_hash)
        except psycopg.errors.UniqueViolation:
            logger.info("duplicate content hash detected for asset %s, deduplicating", asset_id)
...
def handle_deduplication(content_hash: str, pg_pool: PgPool, new_asset_id: str):
    with pg_pool.get_pg_conn() as conn:
        ...
        cur.execute(
            "UPDATE assets SET status = 'ready' WHERE asset_id = %s",
            (new_asset_id,)
        )
