...
def process_image_file(
    asset_id, local_raw_path, pg_pool: PgPool, storage: StorageX, cfg
):
    ...
    with Image.open(local_raw_path) as img:
        ...
        logger.info(
            "image %s opened: format=%s size=%dx%d", asset_id, img.format, width, height
        )
        ...
        with pg_pool.get_pg_conn() as conn:
            cur = conn.cursor()
            cur.execute(
                "UPDATE assets SET width = %s, height = %s, mime_type = %s, updated_at=now() WHERE asset_id=%s",
        ...
        for role, w, h, crop, fmt in IMAGE_VARIANTS:
            try:
                ...
                with pg_pool.get_pg_conn() as conn:
                    cur = conn.cursor()
                    cur.execute(
                        "SELECT 1 FROM variants.image WHERE asset_id=%s AND role=%s",
                        (asset_id, role),
                    )
                    if cur.fetchone():
                        logger.info(
                            "variant %s exists for asset %s -> skip", role, asset_id
                        )
                        continue
                ...
                if crop:
                    ...
                    img_c = ImageOps.fit(
                        img, (w, h), Image.LANCZOS, centering=(0.5, 0.5)
                    )
                else:
                    ...
                    logger.debug(
                        "resized image for role %s to %dx%d",
                        role,
                        img_c.width,
                        img_c.height,
                    )
                ...
                logger.info(
                    "uploaded variant %s for asset %s to %s", role, asset_id, url
                )
